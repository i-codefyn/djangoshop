...
class CatalogueDashboardConfig(OscarDashboardConfig):
    ...
    def ready(self):
        ...
        self.category_create_view = get_class(
            "catalogue_dashboard.views", "CategoryCreateView"
        )
    ...
    def get_urls(self):
        urls = [
            ...
            url(
                r"^products/(?P<parent_pk>[-\d]+)/create-variant/$",
                self.product_createupdate_view.as_view(),
            ...
            url(r"^$", self.product_list_view.as_view(), name="catalogue-product-list"),
            url(
                r"^stock-alerts/$",
            ...
            url(
                r"^categories/create/(?P<parent>\d+)$",
                self.category_create_view.as_view(),
            ...
            url(
                r"^attribute-option-group/(?P<pk>\w+)/update/$",
            ...
            # placeholder urls, so it does need to match more than just a pk
            url(
                r"^attribute-option-group/(?P<pk>\w+)/delete/$",
