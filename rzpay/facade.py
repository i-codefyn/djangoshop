...
def update_transaction_details(rz_id, txn_id):
    ...
    try:
        ...
        logger.warning(
            "Unable to fetch transaction details for rz txn %s: %s", rz_id, e
        )
        raise RazorpayError
    try:
        txn = Transaction.objects.get(txnid=txn_id)
    except Transaction.DoesNotExist as e:
        logger.warning("Unable to find transaction details for txnid %s: %s", txn_id, e)
        raise RazorpayError
    if (
        int(txn.amount * 100) != payment["amount"]
        or txn.currency != payment["currency"]
    ):
        logger.warning("Payment details mismatch for txn %s and %s", txn, payment)
        raise RazorpayError
...
def capture_transaction(rz_id):
    ...
    try:
        ...
        logger.warning("Couldn't capture payment for txn %s: %s", txn, e)
        raise RazorpayError
    return txn
...
def refund_transaction(rz_id, amount, currency):
    try:
        ...
        logger.warning("Couldn't refund txn %s: %s", txn, e)
